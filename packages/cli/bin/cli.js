...
const DEFAULT_SERVER_URL = process.env.SERVER_URL ?? "https://imgcompare.lachlan-miller.me/";
...
export async function maybeGetGitInfo() {
    try {
        ...
        debug("Use is not using git, or another error in getting git info. Ignoring. Error was %s", e);
    }
}
...
async function saveToken(token, projectId) {
    let existing = { token };
    if (fs.existsSync(TOKEN_PATH)) {
        try {
...
async function maybeReadLocalConfig() {
    try {
        const data = await fs.promises.readFile(path.join(process.cwd(), "config.json"), "utf-8");
...
async function init() {
    ...
    await fs.promises.writeFile(configPath, JSON.stringify({ projectId, serverUrl }, null, 2), "utf-8");
    console.log(`
...
async function createRun(projectId, gitinfo) {
    ...
    try {
        const res = await (await api.post(`projects/${projectId}/runs`, {
            json: { gitinfo },
        })).json();
        return res;
    }
